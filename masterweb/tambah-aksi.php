...
$id_datas=rand(1,300);
$cek_id= "SELECT * FROM datas where id_datas ='$id_datas' ";
...
while($numrow > 0){
    ...
    $nama='kasus'.$id_datas;
    $input_datas = "INSERT INTO datas VALUES('$id_datas','$nama') ";
    $datas_result = $db->query($input_datas);
    for($i=1;$i<=$jml_kriteria;$i++){
        $a=$nokriteria[$i];
        $b=$tampung[$i];
    $input_data = "INSERT INTO data (id_data,id_datas,id_kriteria,id_parameter) VALUES(NULL,'$id_datas','$a','$b') ";
